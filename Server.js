...
export default async function (ctx) {
  ...
  try {
    ...
    // 安全数字解析：核心修复手段是各 awk 命令前的全局 LC_ALL=C
    // （见上方 fullCmd）。需要说明：awk 处理大数字时默认会用科学计数法
    // 输出（如 "8.7e+09"），这本身与 locale 无关，是 awk 的固有行为；
    // 但 locale 会进一步影响这个数字的小数点/千分位符号——例如德语等
    // locale 下会把小数点显示成逗号（"8,7e+09"），这种格式 JS 的
    // Number() 无法解析，会直接返回 NaN。LC_ALL=C 强制小数点固定为英文
    // 句点，使标准科学计数法字符串（如 "8.7e+09"）可以被 Number()
    // 正确解析为原始数值，不会有精度损失。
    // 下面的 safeNum 是在此基础上的轻量兜底——仅处理纯千位分隔符场景
    // （如 "1,234,567"）。如果 LC_ALL=C 未生效导致小数点仍被篡改成逗号
    // （如 "8,7e+09"），这种损坏无法安全还原，宁可 fallback 为 0，也不
    // 尝试猜测修复，避免算出看似正常但实际错误的数值。
    const safeNum = (s) => {
      const n = Number(s);
      if (!isNaN(n)) return n;
